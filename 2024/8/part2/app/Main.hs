...
getAntinodes xs x y = 
    let rightMostX = max (fst (xs !! 0)) (fst (xs !! 1))
        ...
        generateAntinodes = 
            ...
            then \i -> 
                [ (rightMostX + i * diffX, downMostY + i * diffY),
                  (leftMostX - i * diffX, upMostY - i * diffY)
                ]
            else \i ->
                [ (rightMostX + i * diffX, upMostY - i * diffY),
                  (leftMostX - i * diffX, downMostY + i * diffY)
                ]
        
        allAntinodes = concatMap generateAntinodes [1..100] --should ideally just do recursively until out of bounds but yea
    in filter isValid (allAntinodes ++ xs)
